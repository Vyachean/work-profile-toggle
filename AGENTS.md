...
- Schedule and background execution are planned product functionality, but must be implemented only after the setup and manual-control UX is clear.
...
- Do not silently enable scheduling unless the app has the Android capabilities needed to run it reliably.

## Implementation standards

- Work in small PRs with one clear product state per PR.
- Plan user flow before implementing background scheduling.
